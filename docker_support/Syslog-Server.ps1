...
$EnableMessageValidation = $false   # Enable check of the PRI and Header
$EnableLocalLogging = $False        # Enable local logging of received messages
...
Function StartReceive([Net.Sockets.Socket]$Socket)
{
  ...
  While ($ServerRunning -eq $True)
  {
    ...
    $HostName = GetHostName $SenderEndPoint.Address.IPAddressToString
    ...
    {
        $MessageString = "Other: <$Facility / $Severity> $MessageString"
    }
    switch($Severity)
    {
        Emergency
